...
struct ContentView: View {
            ...
            List {
                ...
                Section {
                    ForEach(usedWords, id: \.self) { word in
                        HStack {
                            Image(systemName: "\(word.count).circle.fill") // go up to 50
                            Text(word)
    ...
    func addNewWord() {
        ...
        guard isPossible(word: answer) else {
            wordError(title: "Word is not possible", message: "You can't spell \(answer) from \(rootWord)!")
            return
        }
        
        guard isReal(word: answer) else {
            wordError(title: "Word is not real", message: "Come up with a real word.")
            return
        }
